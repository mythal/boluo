...
export const SidebarGroupHeader: FC<Props> = ({ folded, toggle, children, disabled = false, icon: Icon }) => {
  return (
    <button
      onClick={toggle}
      className="flex gap-2 items-center justify-between w-full text-surface-600 py-3 px-4  border-surface-200 group cursor-pointer hover:bg-surface-100"
    >
      ...
      {!disabled && (
        <span
          className={clsx(
            'p-1.5 border rounded-md bg-surface-50 text-sm',
