...
export const ResetPassword = () => {
  ...
  const id = useId();
  const [pageState, setPageState] = useState<'FORM' | 'SUCCESS' | 'NOT_FOUND' | 'UNKNOWN_ERROR'>('FORM');
  const intl = useIntl();
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 pt-2">
      <div>
        ...
        <div className="">
          <TextInput id={id} type="email" className="w-full" {...register('email', { ...validators.required(intl) })} />
        </div>
      </div>
