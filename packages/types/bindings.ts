...
export type AccessPolicy = 'PUBLIC' | 'COLLABORATIVE' | 'PERSONAL' | 'SECRET' | 'GAME_MASTER';

export type AddChannelMember = {
  channelId: string;
  userId: string;
...
export type ChannelWithMaybeMember = {
  channel: Channel;
  member: ChannelMember | null;
};

export type ChannelWithMember = {
  channel: Channel;
  member: ChannelMember;
};
...
export type CheckEntryIdentifier = {
  spaceId: string;
  scopeId: string;
  identifier: string;
};
...
export type EditSpace = {
  ...
  grantAdmins?: string[];
  removeAdmins?: string[];
  grantGameMasters?: string[];
  removeGameMasters?: string[];
};
...
export type EntryComponentHistoryAction = 'SET' | 'REMOVE';

export type EntryComponentHistoryQuery = {
  spaceId: string;
  scopeId: string;
  entryId?: string | null;
  key?: string | null;
...
export type EntryHistoryAction = 'CREATE' | 'RENAME' | 'DELETE';

export type EntryHistoryQuery = {
  spaceId: string;
  scopeId: string;
  entryId?: string | null;
};
...
export type NoteContentRevision = {
  noteId: string;
  revision: number;
  operatorId: string | null;
  title: string;
...
export type ScopeKind = 'SPACE' | 'CHARACTER';
...
export type SpanWithChild = {
  child: ChildText;
} & Span;

export type StatusKind = 'OFFLINE' | 'AWAY' | 'ONLINE';

export type SubExprResult = {
  node: ExprNode;
