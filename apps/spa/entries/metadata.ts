...
export const normalizeEntryDisplayName = (displayName: string): string =>
  displayName.trim().normalize('NFC');

export const isValidEntryDisplayName = (displayName: string): boolean => {
  const length = Array.from(normalizeEntryDisplayName(displayName)).length;
  return length <= ENTRY_DISPLAY_NAME_MAX_LENGTH;
};
