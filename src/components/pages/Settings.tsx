...
import { alignRight, flex, floatRight, largeInput, mR, mT, mY, spacingN, textLg } from '../../styles/atoms';
...
function Settings({ profile }: Props) {
  ...
  return (
    <>
      ...
      <Separator css={[mY(2)]} />
      {updated && (
        <InformationBar variant="SUCCESS" dismiss={() => setUpdated(false)}>
          设置已更新
        </InformationBar>
      )}
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div css={[flex, mT(4)]}>
          ...
          <EditAvatar css={[floatRight]} selectFile={setAvatarFile} mediaId={profile.user.avatarId} />
        </div>
        <div>
          <Label htmlFor="bio">简介</Label>
          <TextArea css={[]} defaultValue={profile.user.bio} id="bio" name="bio" ref={register(bioValidation)} />
        </div>
        <div css={[alignRight]}>
