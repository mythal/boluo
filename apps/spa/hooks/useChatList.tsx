...
interface MakeChatListReturn {
  chatList: ChatItem[];
  filteredMessagesCount: number;
  optimisticMessageMap: Record<string, OptimisticMessage>;
}

const isChatListChanged = (a: MakeChatListReturn, b: MakeChatListReturn) => {
  if (a.chatList.length !== b.chatList.length) return true;
  if (a.chatList !== b.chatList) {
    for (let i = 0; i < a.chatList.length; i++) {
      if (a.chatList[i] !== b.chatList[i]) return true;
    }
  }
  return (
    a.filteredMessagesCount !== b.filteredMessagesCount ||
    a.optimisticMessageMap !== b.optimisticMessageMap
  );
};
...
const makeChatList = (
  ...
  const { messages, previewMap, optimisticMessageMap, fullLoaded } = channelSlice;
  if (!messages || !previewMap)
    return { chatList: [], filteredMessagesCount: 0, optimisticMessageMap };
  ...
  return { chatList: itemList, filteredMessagesCount, optimisticMessageMap };
};
...
export const useChatList = (channelId: string, myId?: string): UseChatListReturn => {
  ...
  const { chatList, filteredMessagesCount, optimisticMessageMap } = useAtomValue(chatListAtom);
