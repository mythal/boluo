...
const IfYou: React.FC<{ signingUp: boolean; toggle: () => void }> = ({ signingUp, toggle }) => {
  return signingUp ? (
    <p className="p-1 text-xs">
      如果已经有账号了，请
      <button onClick={toggle} className="btn-large m-1 px-1 rounded">
        登录
      </button>
      吧。
    </p>
    ...
    <Dialog dismiss={dismiss}>
      <form className="dialog" onSubmit={handleSubmit}>
        ...
        <div className="my-3 text-right">
          <button className="btn-large mr-1" type="button" onClick={dismiss}>
            取消
          </button>
          <button type="submit" disabled={!canSubmit} className="btn-large btn-primary">
            {signingUp ? '注册' : '登录'}
          </button>
        </div>
